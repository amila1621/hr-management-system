...
<div class="content-page">
                            ...
                            <div class="tab-content" id="departmentTabContent">
                                ...
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <form action="{{ route('supervisor.working-hours.store') }}" method="POST" class="department-form">
                                            @csrf
    ...
    <td class="{{ in_array($dateString, $holidays->toArray()) ? '' : '' }}">
        <input type="text" name="reception[{{ $dateString }}]"
            value="{{ isset($organizedReceptionData[$dateString][$department]) ? $organizedReceptionData[$dateString][$department] : '' }}"
    ...
    @endforeach
                                                        ...
                                                        <tr data-staff-id="{{ $staff->id }}" data-staff-email="{{ $staff->email }}">
                                                            ...
                                                            <td class="{{ in_array($dateString, $holidays->toArray()) ? '' : '' }}">
                                                                ...
                                                                <div class="time-slots {{ $isApproved == 0 ? 'unapproved-hours' : '' }}">
                                                                    @forelse($hoursData as $index => $timeRange)
                                                                    <div class="time-slot mb-1 {{ $isApproved == 0 ? 'unapproved-entry' : '' }}">
                                                                        <div class="time-input-container {{ $isApproved == 0 ? 'unapproved-container' : '' }}">
                                                                            @php
                                                                            ...
                                                                            <input type="hidden"
                                                                                ...
                                                                                value="{{ $hiddenValue }}"
                                                                                class="form-control form-control-sm time-range {{ isset($timeRange['type']) && $timeRange['type'] === 'SL' ? 'sick-leave-entry' : '' }} {{ $isOwnRoster ? 'own-roster-disabled' : '' }} {{ $isApproved == 0 ? 'unapproved-input' : '' }}"
