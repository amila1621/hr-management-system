                ...
                <div class="page-title-box mb-4">
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-12">
                            ...
                            <div>
                                <form id="monthSelectForm" action="{{ route('staff.schedule') }}" method="GET">
                                ...
                                <ul class="nav nav-pills nav-justified" role="tablist">
                                    @foreach($departments as $department)
                                        <li class="nav-item waves-effect waves-light">
                                            <a class="nav-link {{ $selectedDepartment === $department ? 'active' : '' }}" 
                                               href="{{ route('staff.schedule', array_merge(request()->query(), ['department' => $department])) }}"
                                               role="tab">
                                    ...
                                    <table class="table table-hover mb-0" id="workingHoursTable">
                                        <thead class="thead-dark">
                                            <tr>
                                                ...
                                                @for ($day = 1; $day <= $daysInMonth; $day++)
                                                    ...
                                                    @endphp
                                                    <th class="day-column {{ $isHoliday ? 'holiday-column' : ($isWeekend ? 'weekend-column' : '') }}" 
                                                        data-date="{{ $dateString }}">
                                                        <div class="day-header">
                                                            <div class="day-number">{{ $day }}</div>
                                                            <div class="day-name d-none d-sm-block">{{ $dayOfWeek }}</div>
                                                        </div>
                                                    </th>
                                                @endfor
                                            </tr>
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="special-row reception-row">
                                                ...
                                                @for ($day = 1; $day <= $daysInMonth; $day++)
                                                    @php
                                                        ...
                                                        $isHoliday = in_array($dateString, $holidays->toArray());
                                                    @endphp
                                                    <td class="time-cell {{ $isHoliday ? 'holiday-cell' : '' }}">
                                                        ...
                                                        <div class="time-content midnight-assignment">
                                                            @if($staffName)
                                                                <span class="assigned-staff">
                                                                    <span class="d-none d-md-inline">{{ $staffName }}</span>
                                                                    <span class="d-md-none">{{ substr($staffName, 0, 1) }}</span>
                                                                </span>
                                                            @endif
                                                        </div>
                                                    </td>
                                            ...
                                            @foreach($staffMembers as $staffMember)
                                                <tr class="staff-row" data-staff-id="{{ $staffMember->id }}">
                                                    <td class="sticky-column staff-name">
                                                        <div class="staff-info">
                                                            ...
                                                            <div class="staff-details">
                                                                <div class="name">
                                                                    <span class="d-none d-md-inline">{{ $staffMember->name }}</span>
                                                                    <span class="d-md-none">{{ substr($staffMember->name, 0, 10) }}{{ strlen($staffMember->name) > 10 ? '...' : '' }}</span>
                                                                </div>
                                                                <div class="role d-none d-md-block">{{ $staffMember->department ?? 'Staff' }}</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    @for ($day = 1; $day <= $daysInMonth; $day++)
                                                        ...
                                                        @endphp
                                                        <td class="time-cell {{ $isHoliday ? 'holiday-cell' : '' }}" 
                                                            data-staff="{{ $staffMember->id }}" 
                                                            data-date="{{ $dateString }}">
                                                            <div class="time-content">
                                                                @forelse($timeValues as $timeValue)
                                                                    @if(is_array($timeValue))
                                                                        @if(isset($timeValue['type']) && in_array($timeValue['type'], ['V', 'X', 'H']))
                                                                            <div class="time-entry day-off-entry {{ strtolower($timeValue['type']) }}-type">
                                                                                <span class="day-off-badge">{{ $timeValue['type'] }}</span>
                                                                            </div>
                                                                        @elseif(isset($timeValue['type']) && $timeValue['type'] === 'SL')
                                                                            <div class="time-entry sick-leave-entry">
                                                                                <i class="fas fa-user-injured mr-1 d-none d-md-inline"></i>
                                                                                <span class="time-text">SL</span>
                                                                                @if(isset($timeValue['start_time']) && isset($timeValue['end_time']))
                                                                                    <div class="time-details d-none d-md-block">{{ $timeValue['start_time'] }} - {{ $timeValue['end_time'] }}</div>
                                                                                @endif
                                                                            </div>
                                                                        @elseif(isset($timeValue['type']) && $timeValue['type'] === 'reception')
                                                                            <div class="time-entry reception-entry">
                                                                                ...
                                                                                @if(isset($timeValue['start_time']) && isset($timeValue['end_time']))
                                                                                    <div class="time-details">
                                                                                        <span class="d-none d-md-inline">{{ $timeValue['start_time'] }} - {{ $timeValue['end_time'] }}</span>
                                                                                        <span class="d-md-none">
                                                                                            {{ substr($timeValue['start_time'], 0, 5) }}<br>{{ substr($timeValue['end_time'], 0, 5) }}
                                                                                        </span>
                                                                                    </div>
                                                                                @endif
                                                                            </div>
                                                                        @elseif(isset($timeValue['type']) && $timeValue['type'] === 'on_call')
                                                                    ...
                                                                    @elseif(is_string($timeValue))
                                                                        @if(in_array($timeValue, ['V', 'X', 'H']))
                                                                            <div class="time-entry day-off-entry {{ strtolower($timeValue) }}-type">
                                                                                <span class="day-off-badge">{{ $timeValue }}</span>
                                                                            </div>
                                                                        @elseif($timeValue === 'SL')
                                                                            <div class="time-entry sick-leave-entry">
                                                                                <i class="fas fa-user-injured mr-1 d-none d-md-inline"></i>
                                                                        ...
                                                                        @elseif(strpos($timeValue, '-') !== false)
                                                                            ...
                                                                            <div class="time-entry work-entry">
                                                                                <i class="fas fa-clock mr-1 d-none d-md-inline"></i>
                                                                                <span class="time-text">
                                                                                    <span class="d-none d-md-inline">{{ trim($start) }} - {{ trim($end) }}</span>
                                                                                    <span class="d-md-none">
                                                                                        @php
                                                                                        ...
                                                                                        @endphp
                                                                                        {{ $startFormatted }}<br>{{ $endFormatted }}
                                                                                    </span>
                                                                                </span>
                                                                            </div>
                                                                        @else
